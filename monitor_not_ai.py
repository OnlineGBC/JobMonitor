            ...
            if linkedin_url:
                ...
                if auth_block_detected:
                    try:
                        ...
                        dump_path = Path("logs") / f"{name}_auth_dump_{timestamp}.html"
                        dump_path.write_text(html, encoding="utf-8", errors="ignore")
                        logging.warning(f"[{name}] Auth/challenge HTML dumped to {dump_path.resolve()}")
                    except Exception as dump_error:
                        logging.warning(f"[{name}] Failed to write auth HTML dump: {dump_error}")
